var Harpy=function(){function e(a){return-1!==a.indexOf("image")?"image":-1!==a.indexOf("html")?"html":-1!==a.indexOf("javascript")?"js":-1!==a.indexOf("css")?"css":-1!==a.indexOf("text")?"text":"other"}function d(a,c){switch(c){case "size":return 1024<a?(a=Math.round(a/102.4),a/10+" KB"):a+" B";case "time":return 1E3<a?(a=Math.round(a/10),a/100+" s"):a+" ms"}return a}function k(a,c,d){var f=a.length>c?"...":"";return d?f+(a.length>c?a.substr(a.length-c):a):a.substr(0,c)+f}function r(){return{time:{blocked:0,
    ...
    var f=this,m={},b=null;a=JSON.parse(a);var q=a.log.pages[0].pageTimings;q.total=q.onLoad;var t=new Date(a.log.pages[0].startedDateTime),h=r(),u,g,v,s,x,w;for(u=0;u<a.log.entries.length;u++){g=a.log.entries[u];g.harpy_info={};m[u]=g;v=g.request.url;g.harpy_info.source=-1<v.indexOf(a.log.entries[0].request.url)?"internal":"external";0<u&&(v=v.replace(a.log.entries[0].request.url,""));s=e(g.response.content.mimeType);x=(new Date(g.startedDateTime)).getTime()+g.time;x>t.getTime()+q.total&&(q.total=x-
        t);for(w in g.timings)g.timings.hasOwnProperty(w)&&0<g.timings[w]&&(h.time[w]+=g.timings[w]);h.type[s]+=g.response.bodySize;h.uncached[s]+=g.response.bodySize;h.size.download+=g.response.bodySize;g.harpy_info.index=u+1;g.harpy_info.url=v;g.harpy_info.mimetype=s;g.harpy_info.cache="(cache)"===g.response.status?" cache":"";g.cache.hasOwnProperty("afterRequest")?(h.size.cache+=g.response.content.size,h.type[s]+=g.response.content.size,g.harpy_info.size=g.response.content.size):g.harpy_info.size=g.response.bodySize;
        g.harpy_info.startTime=new Date(g.startedDateTime)-t}t={};t.time=q;t.time.onContentLoad=q.onContentLoad||q.onLoad;t.size=h.size;a.log.pages[0].harpy_info=t;this.resize=c;this.showInfo=function(a){var b=a.getAttribute("data-id");g=m[b];g.info?g.info.style.display="none"===g.info.style.display?"table-row":"none":(b=document.createElement("TR"),b.innerHTML=Harpy.Templates.info(g),$(a).after(b),g.info=b);c()};this.draw=function(c){this.el=b=c;google.visualization?p():(google.load("visualization","1",
        {packages:["corechart"]}),google.setOnLoadCallback(p));c={};c["#"+b+" table.harpy>tbody>tr"]={click:function(a,b){f.showInfo(b)}};Exos.enable([c,{window:{resize:function(){f.resize()}}}]);Handlebars.registerHelper("trimmer",k);Handlebars.registerHelper("format",d);Handlebars.registerPartial("timeline",Harpy.Templates.timeline);c=document.createElement("TABLE");c.innerHTML=Harpy.Templates.data(a);c.className="harpy";document.getElementById(b).appendChild(c);setTimeout(this.resize,1);$.tablesorter&&
    $(c).tablesorter({sortList:[[0,0]]})}},Comparator:function(c,d){function p(a){var b,c,d,p,f,m,g=a.log.pages[0].pageTimings;g.total=g.onLoad;var k=new Date(a.log.pages[0].startedDateTime),h=r();for(c=0;c<a.log.entries.length;c++){b=a.log.entries[c];d=b.request.url;p=e(b.response.content.mimeType);f=(new Date(b.startedDateTime)).getTime()+b.time;f>k.getTime()+g.total&&(g.total=f-k);for(m in b.timings)b.timings.hasOwnProperty(m)&&0<b.timings[m]&&(h.time[m]+=b.timings[m]);h.type[p]+=b.response.bodySize;
    h.uncached[p]+=b.response.bodySize;h.size.download+=b.response.bodySize;0<c&&d.replace(a.log.entries[0].request.url,"");b.cache.hasOwnProperty("afterRequest")&&(h.size.cache+=b.response.content.size,h.type[p]+=b.response.content.size)}return h}function f(a){a=a.log.entries;var b=0,c;for(c=a.length-1;0<=c;c--)"(cache)"!==a[c].response.status&&(b+=1);return b}function m(){l(b,k,t);a(b,w);a(b,x)}c=JSON.parse(c);d=JSON.parse(d);var b,k=p(c),t=p(d),h=f(c),u=f(d),g=c.log.pages[0].pageTimings,v=d.log.pages[0].pageTimings,
    s=["Metric","Uncached","Primed cache"],x=[s,["Dom load",v.onContentLoad/1E3,g.onContentLoad/1E3],["Load",v.onLoad/1E3,g.onLoad/1E3],["Total",v.total/1E3,g.total/1E3]],w=[s,["Requests",u,h]];this.draw=function(a){this.el=b=a;google.visualization?m():(google.load("visualization","1",{packages:["corechart"]}),google.setOnLoadCallback(m))}}}}();this.Harpy=this.Harpy||{};this.Harpy.Templates=this.Harpy.Templates||{};
this.Harpy.Templates.data=Handlebars.template(function(e,d,k,r,l){this.compilerInfo=[4,">= 1.0.0"];k=this.merge(k,e.helpers);r=this.merge(r,e.partials);l=l||{};var a,c,f=this.escapeExpression,n=k.helperMissing,y=this;e=""+('<thead>\r\n    <tr>\r\n        <th></th>\r\n        <th>Method</th>\r\n        <th>URL</th>\r\n        <th>Status</th>\r\n        <th>Type</th>\r\n        <th>Size</th>\r\n        <th class="timeline">\r\n            <div class="loadMarker" data-dom="'+f((a=(a=(a=(a=(a=(a=d&&d.log,
